...
namespace AcupunctureProject.Database
{
    ...
    public class DatabaseConnection
    {
        ...
        private SQLiteCommand FindTreatmentsSt;
        ...
        private DatabaseConnection()
        {
            ...
            FindTreatmentsSt = new SQLiteCommand(Connection);
        ...
        public List<Treatment> FindTreatments(string name)
        {
            FindTreatmentsSt.CommandText = "select * from TREATMENT where NAME like '%" + name + "%';";
            using (SQLiteDataReader rs = FindTreatmentsSt.ExecuteReader())
            {
                return GetTreatments(rs);
            }
        }
        ...
        public Treatment InsertTreatment(Treatment treatment)
        {
            ...
            if (rowId != 0)
            {
                ...
                return new Treatment(id, treatment);
            }
            throw new Exception("ERORR:insert point did't accure");
        }
        #endregion
        ...
        private List<Treatment> GetTreatments(SQLiteDataReader rs)
        {
            List<Treatment> o = new List<Treatment>();
            while (rs.Read())
            {
        ...
        private Treatment GetTreatment(SQLiteDataReader rs)
        {
            return new Treatment(rs.GetIntL(ID), rs.GetStringL(Treatment.NAME), rs.GetStringL(Treatment.PATH));
        }
